...
Runs, not distinct patterns: the small exhaustive boxes are contained in the 5x5 box, the random
draws repeat in small boxes, and the evolutionary search re-evaluated some genomes, so the number
of distinct starting patterns is smaller. An upper bound is 40,395,921 (33,554,432 exhaustive +
at most 4,163,399 random draws outside the 5x5 box + at most 2,678,090 adversarial); the exact
union was not computed. Of the adversarial runs, 21,861 are regenerated by the committed scripts; the
